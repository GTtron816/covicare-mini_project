...
$query =mysqli_query( $conn," SELECT no,hspname,addrs,bed,oxycl,lat,lon FROM hospitals where place='".$place."'");
while ($data = mysqli_fetch_assoc($query))  
{
 ...
 echo '
            ...
            <td> '.$lat.'</td>
            <td> '.$lon.'</td>
            <td><button class="btde" onclick=uphs('.$no.')><i class="fas fa-pen-alt"></i> UPDATE</button></td>
            <td><button class="btde" onclick=remhs('.$no.')><i class="fas fa-minus-circle"></i> REMOVE</button></td>
           </tr>
...
<form method="post" action="updatehs.php" id="myform">
...
document.getElementById("myform").submit();
     ...
     <form method="post" action="remhsp.php" id="form1">
